...
function initializeCollectionLog(player: PlayerState, services: CollectionLogOpenData): void {
    ...
    populateCollectionLogCategories(player, 0, {
        queueVarp: (pid, id, val) => services.queueVarp(pid, id, val),
        queueVarbit: (pid, id, val) => services.queueVarbit(pid, id, val),
        queueWidgetEvent: (pid, evt) => services.queueWidgetEvent(pid, evt as any),
        queueNotification: () => {},
        queueChatMessage: () => {},
