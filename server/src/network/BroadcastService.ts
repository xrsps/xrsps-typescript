...
export class BroadcastService {
    ...
    broadcast(msg: string | Uint8Array, context = "broadcast"): void {
        const networkLayer = this.svc.networkLayer;
        for (const client of this.svc.wssClients) {
    ...
    enqueueForcedMovement(event: ForcedMovementBroadcast): void {
        ...
        if (frame) {
            ...
            this.svc.broadcastScheduler.queueForcedMovement(event);
        }
    }
    ...
    queueSpellResult(playerId: number, payload: SpellResultPayload): void {
        ...
        this.svc.broadcastScheduler.queueSpellResult(playerId, payload);
    }

    queueSmithingInterfaceMessage(playerId: number, payload: SmithingServerPayload): void {
        this.svc.broadcastScheduler.queueKeyedMessage("smithing", playerId, payload);
    }
