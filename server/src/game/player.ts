...
export class PlayerState extends Actor {
    ...
    setLevelUpTaskCallback(
        ...
        this.levelUpTaskCallback = callback;
    }
    ...
    setSkillXp(id: SkillId, xp: number): void {
        ...
        if (baseChanged && skill.baseLevel > prevBase) {
            this.levelUpTaskCallback?.(this, id, prevBase, skill.baseLevel);
        }
...
export class PlayerManager implements PlayerRepository {
    ...
    setLocInteractionCallback(
        ...
        this.interactionSystem.setLocInteractionCallback(callback);
    }
    ...
    setLevelUpTaskCallback(
        ...
        for (const player of this.players.values()) {
            player.setLevelUpTaskCallback(callback);
        }

        for (const bot of this.bots) {
            bot.setLevelUpTaskCallback(callback);
        }

        for (const orphan of this.orphanedPlayers.values()) {
            orphan.player.setLevelUpTaskCallback(callback);
        }
    }
    ...
    add(ws: any, spawnX: number, spawnY: number, level: number = 0): PlayerState | undefined {
        ...
        p.setLevelUpTaskCallback(this.levelUpTaskCallback);
