...
export class Pathfinder {
    ...
    setNpcFlags(
        ...
        srcSize: number = 1,
        clearFlag: number = CollisionFlag.BLOCK_NPCS,
    ) {
        ...
        // Self-avoidance: clear dynamic blocker for the origin footprint so the NPC
        // does not block itself when planning the first step.
        const size = Math.max(1, srcSize);
        for (let fx = 0; fx < size; fx++) {
            for (let fy = 0; fy < size; fy++) {
                const gx = srcX + fx - graphBaseX;
                const gy = srcY + fy - graphBaseY;
                if (gx >= 0 && gy >= 0 && gx < this.graphSize && gy < this.graphSize) {
                    this.flags[gx][gy] &= ~clearFlag;
                }
            }
        }
    }
    ...
    findPath(
        ...
        if (!found && findAlternative) {
            ...
            const approxSizeX = Math.max(1, routeStrategy.destSizeX);
            const approxSizeY = Math.max(1, routeStrategy.destSizeY);
            ...
            for (
                ...
                for (
                    ...
                    } else if (checkX > approxDestX + approxSizeX - 1) {
                        deltaX = checkX - (approxDestX + approxSizeX - 1);
                    }

                    let deltaY = 0;
                    if (checkY < approxDestY) {
                        deltaY = approxDestY - checkY;
                    } else if (checkY > approxDestY + approxSizeY - 1) {
                        deltaY = checkY - (approxDestY + approxSizeY - 1);
                    }

                    const cost = deltaX * deltaX + deltaY * deltaY;
                    const distance = this.distances[graphX][graphY];
