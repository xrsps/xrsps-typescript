...
export class ObjType extends Type {
    ...
    // Wear offsets (applied to wearable models on the actor)
    manwearXOff: number;
    manwearYOff: number;
    manwearZOff: number;
    ...
    override decodeOpcode(opcode: number, buffer: ByteBuffer): void {
        if (opcode === 1) {
            ...
            this.examine = this.readString(buffer);
        } else if (opcode === 4) {
            ...
            // OSRS newer: short value placeholder (e.g., map icon or similar metadata)
            ...
            const raw = buffer.readUnsignedShort();
            this.resizeX = this.normalizeResize(raw);
        } else if (opcode === 111) {
            const raw = buffer.readUnsignedShort();
            this.resizeY = this.normalizeResize(raw);
        } else if (opcode === 112) {
            const raw = buffer.readUnsignedShort();
            this.resizeZ = this.normalizeResize(raw);
        } else if (opcode === 113) {
    ...
    private normalizeResize(raw: number): number {
        // Some newer cache revisions appear to emit garbage resize values when the
        // object decoder encounters unknown opcodes.  Legitimate resize factors in
        // the live client stay within a small multiple of the default 128.  If we
        // see anything wildly outside that range, fall back to the vanilla scale
        // so that wearable models do not explode to thousands of units.
        if (raw <= 0 || raw > 1024) {
            return 128;
        }
        return raw;
