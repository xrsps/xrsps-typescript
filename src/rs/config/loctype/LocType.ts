...
export class LocType extends Type {
    ...
    soundAreaRadiusOverride: number;
    hasSpatialAmbient: boolean;
    loopMultiSoundSequentially: boolean;
    useExactSoundPosition: boolean;
    deferredAmbientSwap: boolean;
    resetAmbientOnLoopRestart: boolean;
    ...
    constructor(id: number, cacheInfo: CacheInfo) {
        ...
        this.hasSpatialAmbient = false;
        this.loopMultiSoundSequentially = false;
        this.useExactSoundPosition = false;
        this.deferredAmbientSwap = false;
        this.resetAmbientOnLoopRestart = false;
    ...
    override decodeOpcode(opcode: number, buffer: ByteBuffer): void {
        if (this.isNewModelsFormat() && (opcode === 1 || opcode === 5)) {
            ...
            this.deferredAmbientSwap = true;
        } else if (opcode === 91) {
            this.soundDistanceFadeCurve = buffer.readUnsignedByte();
        } else if (opcode === 93) {
            if (this.cacheInfo.game === "oldschool" && this.cacheInfo.revision >= 220) {
                this.soundFadeInCurve = buffer.readUnsignedByte();
                this.soundFadeInDuration = buffer.readUnsignedShort();
                this.soundFadeOutCurve = buffer.readUnsignedByte();
                this.soundFadeOutDuration = buffer.readUnsignedShort();
            } else {
            ...
            this.hasSpatialAmbient = true;
        } else if (opcode === 177) {
            this.loopMultiSoundSequentially = buffer.readUnsignedByte() === 1;
        } else if (opcode === 178) {
            this.soundAreaRadiusOverride = buffer.readUnsignedByte();
        } else if (opcode === 189) {
            this.useExactSoundPosition = true;
        } else if (opcode === 190) {
            this.deferredAmbientSwap = buffer.readUnsignedByte() === 1;
        } else if (opcode === 191) {
            this.resetAmbientOnLoopRestart = buffer.readUnsignedByte() === 1;
        } else if (opcode === 249) {
