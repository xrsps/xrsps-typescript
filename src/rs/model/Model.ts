...
type SkeletalAnimationOptions = {
    ...
    masks?: boolean[];
    /**
     * Whether the provided mask should be treated as "active" (true) or "inactive" (false).
     * Defaults to false so that the first pass targets unmasked bones by default.
     */
    maskMatch?: boolean;
    /**
     * Controls whether alpha curves are applied for this pass. Defaults to `!maskMatch` to
     * replicate OSRS behaviour where alpha is only applied on the base (unmasked) pass.
     */
    applyAlpha?: boolean;
    /**
     * When true, only updates the bone pose matrices. Call `applySkeletalPose` afterwards
     * to run the actual skinning step once all contributing passes have been prepared.
     */
    deferTransform?: boolean;
};

type ApplySkeletalPoseOptions = {
    /** Whether to run alpha curves during the apply step. Defaults to true. */
    applyAlpha?: boolean;
};

type SkeletalOverlayOptions = {
    seq: SkeletalSeq;
    frame: number;
    /** Overlay alpha is not handled currently (mirrors OSRS base-only alpha). */
    applyAlpha?: boolean;
};

type SkeletalCompositeOptions = {
    masks?: boolean[];
    overlay?: SkeletalOverlayOptions;
};
...
export class Model extends Entity {
    ...
    // Color override fields (for damage/poison/freeze tints)
    // Reference: player-animation.md lines 1144-1151
    overrideHue: number = 0; // 0-127
    overrideSaturation: number = 0; // 0-127
    overrideLuminance: number = 0; // 0-127
    overrideAmount: number = 0; // 0-255 (0 = no override, 255 = full override)
    ...
    contourGround(
        ...
        if (type === 1) {
            ...
            const clampLimit = param !== 0 ? Math.abs(param) : undefined;
            for (let i = 0; i < model.usedVertexCount; i++) {
                const vx = this.verticesX[i] + sceneX;
                const vz = this.verticesZ[i] + sceneZ;
                const rx = vx & 0x7f;
                ...
                if (
                    ...
                    const h1 =
                        (heightMap[tx][tz + 1] * (128 - rx) + heightMap[tx + 1][tz + 1] * rx) >> 7;
                    let delta = ((h0 * (128 - rz) + h1 * rz) >> 7) - sceneHeight;
                    if (clampLimit !== undefined && clampLimit > 0) {
                        if (delta > clampLimit) {
                            delta = clampLimit;
                        } else if (delta < -clampLimit) {
                            delta = -clampLimit;
                        }
                    }
                    model.contourVerticesY[i] = this.verticesY[i] + delta;
                } else {
                    model.contourVerticesY[i] = this.verticesY[i];
                }
            }
    ...
    animate(frame: SeqFrame, nextFrame: SeqFrame | undefined, op14: boolean): void {
        if (this.vertexLabels) {
            ...
            this.transformInterpolated(base, frame, nextFrame, undefined, true, op14, 0xffff, 0);
            this.postAnimate();
        }
    }
    ...
    private applyInterleaveFrame(
        ...
        for (let i = 0; i < frame.transformCount; i++) {
            ...
            while (group > maskValue) {
                maskValue = (interleave[maskIndex] ?? 9999999) | 0;
                maskIndex++;
            }

            const type = base.types[group] | 0;
            const selected = match
                ? group === maskValue || type === SeqTransformType.ORIGIN
                : group !== maskValue || type === SeqTransformType.ORIGIN;
    ...
    private applyInterleaveFrameInterpolated(
        ...
        void nextFrame;
        void alpha;
        this.applyInterleaveFrame(frame, interleave, match, op14);
    }
    ...
    animateInterleavedFrame(
        frame: SeqFrame,
        op14: boolean,
        interleave: number[] | undefined,
        match: boolean,
    ): void {
        if (!this.vertexLabels) return;
        if (!interleave || interleave.length === 0) {
            this.animate(frame, undefined, op14);
            return;
        }
        this.applyInterleaveFrame(frame, interleave, match, op14);
        this.postAnimate();
    ...
    animateInterleavedFrames(
        baseFrame: SeqFrame,
        baseOp14: boolean,
        overlayFrame: SeqFrame,
        overlayOp14: boolean,
        interleave: number[],
    ): void {
        if (!this.vertexLabels) return;
        if (!interleave || interleave.length === 0) {
            this.animate(baseFrame, undefined, baseOp14);
            return;
        }
        this.applyInterleaveFrame(baseFrame, interleave, false, baseOp14);
        this.applyInterleaveFrame(overlayFrame, interleave, true, overlayOp14);
        this.postAnimate();
    }

    animateInterleavedFramesInterpolated(
        baseFrame: SeqFrame,
        nextBaseFrame: SeqFrame | undefined,
        baseAlpha: number,
        baseOp14: boolean,
        overlayFrame: SeqFrame,
        nextOverlayFrame: SeqFrame | undefined,
        overlayAlpha: number,
        overlayOp14: boolean,
        interleave: number[],
    ): void {
        if (!this.vertexLabels) return;
        if (!interleave || interleave.length === 0) {
            this.animateInterpolated(baseFrame, nextBaseFrame, baseAlpha, baseOp14);
            return;
        }
        this.applyInterleaveFrameInterpolated(
            baseFrame,
            nextBaseFrame,
            baseAlpha,
            interleave,
    ...
    animateSkeletal(
        ...
        const {
            masks,
            maskMatch = false,
            applyAlpha = !maskMatch,
            deferTransform = false,
        } = options;
        const skeletalBase = skeletalSeq.skeletalBase;
        if (!skeletalBase) {
            if (!deferTransform && applyAlpha && skeletalSeq.hasAlphaTransform) {
        ...
        skeletalBase.updateAnimMatrices(skeletalSeq, frame, masks, maskMatch);

        if (!deferTransform) {
            this.applySkeletalPoseInternal(skeletalSeq, skeletalBase, frame, applyAlpha);
        }
    }
    ...
    applySkeletalPose(
        skeletalSeq: SkeletalSeq,
        frame: number,
        options: ApplySkeletalPoseOptions = {},
    ): void {
        const { applyAlpha = true } = options;
        const skeletalBase = skeletalSeq.skeletalBase;
        if (!skeletalBase) {
            return;
        }
    ...
    animateSkeletalComposite(
        baseSeq: SkeletalSeq,
        baseFrame: number,
        composite: SkeletalCompositeOptions = {},
    ): void {
        const { masks, overlay } = composite;

        if (!masks || masks.length === 0 || !overlay) {
            this.animateSkeletal(baseSeq, baseFrame);
            return;
        }

        if (overlay.seq.skeletalBase !== baseSeq.skeletalBase) {
            // Mixed bases cannot be combined safely; fall back to single-sequence behaviour.
            this.animateSkeletal(baseSeq, baseFrame);
            return;
        }
        ...
        this.animateSkeletal(overlay.seq, overlay.frame, {
            masks,
            maskMatch: true,
            applyAlpha: false,
            deferTransform: true,
        ...
        // Alpha curves follow the base sequence to mirror OSRS behaviour.
        this.applySkeletalPose(baseSeq, baseFrame, { applyAlpha: true });
    }
    ...
    private applySkeletalPoseInternal(
        skeletalSeq: SkeletalSeq,
        ...
        if (applyAlpha && skeletalSeq.hasAlphaTransform) {
            this.transformSkeletalAlpha(skeletalSeq, frame);
        }
    ...
    // Linearly blend this model's vertex positions toward another model's positions.
    // Assumes both models share the same topology and vertex count.
    morphLerp(target: Model, alpha: number): void {
        const t = Math.max(0, Math.min(1, alpha));
        if (t === 0) return;
        const n = Math.min(this.verticesCount | 0, target.verticesCount | 0);
        const ax = this.verticesX,
            ay = this.verticesY,
            az = this.verticesZ;
        const bx = target.verticesX,
            by = target.verticesY,
            bz = target.verticesZ;
        for (let i = 0; i < n; i++) {
            ax[i] = (ax[i] + (bx[i] - ax[i]) * t) | 0;
            ay[i] = (ay[i] + (by[i] - ay[i]) * t) | 0;
            az[i] = (az[i] + (bz[i] - az[i]) * t) | 0;
        }
        this.invalidateBounds();
    }
}
